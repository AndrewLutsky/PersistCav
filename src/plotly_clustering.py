...
def agglomerative_clustering(vectors):
    clustering = AgglomerativeClustering(n_clusters=4).fit(vectors)
...
ids = [labels[0:4] + "_" + labels[4] for labels in pdbs]
print(ids)
vectors, ids = read_vectors("data.csv.npy"), read_labels("vector_labels.csv")
ids = [idx[7:] for idx in ids]
ids = [idx[0:6] for idx in ids]
#distance_matrix = pairwise_distances(vectors, metric='manhattan')
